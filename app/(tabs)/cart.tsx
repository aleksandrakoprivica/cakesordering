            ...
            <View className="flex-1 px-5 pt-6">
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View className="py-20 items-center">
                            ...
                            <Text className="text-gray-500 text-center text-lg font-medium">Your cart is empty</Text>
                            <Text className="text-gray-400 text-center text-sm mt-2">Add some delicious cakes!</Text>
                        </View>
                    }
                    renderItem={({ item }) => (
                        <View className="rounded-3xl border border-gray-200 bg-white p-5 shadow-md">
                            <View className="flex-row items-start justify-between mb-3">
                                <View className="flex-1">
                                    <Text className="text-xl font-bold text-gray-900 mb-1">{item.cakeName}</Text>
                                    {item.sizeLabel ? (
                                        <Text className="text-gray-500 text-sm">Size: {item.sizeLabel}</Text>
                                    ) : (
                                        <Text className="text-gray-500 text-sm">Bento Cake</Text>
                                    )}
                                </View>
                                <Text className="text-lg font-bold text-gray-900 ml-3">{formatRSD(item.unitPriceRsd)}</Text>
                            </View>
                ...
                {items.length > 0 ? (
                    <View className="border-t border-gray-200 bg-white px-5 py-6 mt-4 rounded-t-3xl shadow-lg">
                        ...
                        <Pressable className="rounded-2xl bg-black py-4 active:opacity-90">
